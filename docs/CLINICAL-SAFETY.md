...
| H-75 | Safety check consults one chart of a linked person | The check reads the named chart while the summary assembles across members | The assembled chart shows a penicillin allergy the safety check calls clear — a false negative against the person the link asserts | Catastrophic | Low | The check unions allergies and medications across the members the caller could assemble; a member it could not consult is a blocking finding, never silence | `test/chart-links.test.ts` — "the safety check answers for the person the link asserts"; `test/chart-links.test.ts` — "a withheld member is not consulted by the safety check — and the check says so" |
