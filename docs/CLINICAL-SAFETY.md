...
| R-19 | The offline reading station is a design, not a build (#38) | The staleness vocabulary and its rendering exist (H-83); the station — snapshot fill, serving budget, offline break-glass, audit reconciliation, autonomous expiry — is written in [docs/OFFLINE-CHART.md](OFFLINE-CHART.md) and not in code | Do not serve a cached chart from anything until the station is built to that design; until then an outage means the write queue and paper, which is today's posture |
