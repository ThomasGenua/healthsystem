...
| Broad medication decision-support *content* | The check is here; the licensed interaction table is not. Deterministic published risk instruments are implemented separately and carry an explicit unreviewed assurance state. |
...
| R-04 | Medication decision-support mechanism without broad content (H-20) | A partial interaction table is more dangerous than a small one; deterministic risk scores are a separate governed catalogue | Licence an interaction source, or accept that interactions are unchecked |
